...
void HelioQuery::request_support_machine(const std::string helio_api_url, const std::string helio_api_key, int page)
{
    ...
    std::cerr << "[HELIO DEBUG] request_support_machine called - URL: " << url_copy << ", Page: " << page_copy << std::endl;
    std::string response_headers;
    auto http = Http::post(url_copy);

    http.header("Content-Type", "application/json")
        // Keep Authorization consistent with other Helio GraphQL calls in this codebase.
    ...
    http.timeout_connect(20)
        ...
        .on_complete([url_copy, key_copy, page_copy](std::string body, unsigned status) {
            std::cerr << "[HELIO DEBUG] request_support_machine on_complete - Status: " << status << ", Body length: " << body.length() << std::endl;
            if (body.length() < 500) {
                std::cerr << "[HELIO DEBUG] Response body: " << body << std::endl;
            } else {
                std::cerr << "[HELIO DEBUG] Response body (first 500 chars): " << body.substr(0, 500) << std::endl;
            }
            
            if (status != 200) {
                std::cerr << "[HELIO ERROR] request_support_machine non-200 status: " << status << std::endl;
                return;
            }
                ...
                if (parsed_obj.contains("data") && parsed_obj["data"].contains("printers")) {
                    ...
                    if (materials.contains("objects") && materials["objects"].is_array()) {
                        for (const auto &pobj : materials["objects"]) {
                            ...
                            // For CrealityPrint items, use name directly if no bambustudio alternative name
                            if (sp.native_name.empty() && pobj.contains("name") && !pobj["name"].is_null()) {
                    ...
                    BOOST_LOG_TRIVIAL(warning) << "Helio: Loaded " << supported_printers.size() << " printers (page " << page_copy << "), total: " << HelioQuery::global_supported_printers.size();
                    std::cerr << "[HELIO DEBUG] Loaded " << supported_printers.size() << " printers (page " << page_copy << "), total: " << HelioQuery::global_supported_printers.size() << std::endl;
                    ...
                    if (materials.contains("pageInfo") && materials["pageInfo"].contains("hasNextPage") && materials["pageInfo"]["hasNextPage"].get<bool>()) {
                        ...
                        std::cerr << "[HELIO DEBUG] No more pages for printers" << std::endl;
                    }
                } else {
                    std::cerr << "[HELIO DEBUG] Response doesn't contain expected data structure" << std::endl;
                }
            } catch (const std::exception& e) {
                BOOST_LOG_TRIVIAL(error) << "Helio request_support_machine exception: " << e.what();
                std::cerr << "[HELIO ERROR] request_support_machine exception: " << e.what() << std::endl;
            } catch (...) {
                BOOST_LOG_TRIVIAL(error) << "Helio request_support_machine unknown exception";
                std::cerr << "[HELIO ERROR] request_support_machine unknown exception" << std::endl;
            }
        })
        .on_error([url_copy](std::string body, std::string error, unsigned status) {
            BOOST_LOG_TRIVIAL(error) << "Helio request_support_machine failed - URL: " << url_copy << ", Status: " << status << ", Error: " << error << ", Body: " << body;
            std::cerr << "[HELIO ERROR] request_support_machine failed - URL: " << url_copy << ", Status: " << status << ", Error: " << error << ", Body: " << body << std::endl;
        })
        .perform();
}
...
void HelioQuery::request_support_material(const std::string helio_api_url, const std::string helio_api_key, int page)
{
    ...
    std::cerr << "[HELIO DEBUG] request_support_material called - URL: " << url_copy << ", Page: " << page_copy << std::endl;
    auto http = Http::post(url_copy);
    ...
    http.timeout_connect(20)
        .timeout_max(100)
        .on_complete([url_copy, key_copy, page_copy](std::string body, unsigned status) {
            std::cerr << "[HELIO DEBUG] request_support_material on_complete - Status: " << status << ", Body length: " << body.length() << std::endl;
            if (body.length() < 500) {
                std::cerr << "[HELIO DEBUG] Response body: " << body << std::endl;
            } else {
                std::cerr << "[HELIO DEBUG] Response body (first 500 chars): " << body.substr(0, 500) << std::endl;
            ...
            if (status != 200) {
                std::cerr << "[HELIO ERROR] request_support_material non-200 status: " << status << std::endl;
                return;
            }
            ...
            try {
                if (parsed_obj.contains("data") && parsed_obj["data"].contains("materials")) {
                    ...
                    BOOST_LOG_TRIVIAL(warning) << "Helio: Loaded " << supported_materials.size() << " materials (page " << page_copy << "), total: " << HelioQuery::global_supported_materials.size();
                    std::cerr << "[HELIO DEBUG] Loaded " << supported_materials.size() << " materials (page " << page_copy << "), total: " << HelioQuery::global_supported_materials.size() << std::endl;

                    if (materials.contains("pageInfo") && materials["pageInfo"].contains("hasNextPage") && materials["pageInfo"]["hasNextPage"].get<bool>()) {
                        HelioQuery::request_support_material(url_copy, key_copy, page_copy + 1);
                    } else {
                        std::cerr << "[HELIO DEBUG] No more pages for materials" << std::endl;
                    }
                } else {
                    std::cerr << "[HELIO DEBUG] Response doesn't contain expected data structure for materials" << std::endl;
                }
            } catch (const std::exception& e) {
                BOOST_LOG_TRIVIAL(error) << "Helio request_support_material exception: " << e.what();
                std::cerr << "[HELIO ERROR] request_support_material exception: " << e.what() << std::endl;
            } catch (...) {
                BOOST_LOG_TRIVIAL(error) << "Helio request_support_material unknown exception";
                std::cerr << "[HELIO ERROR] request_support_material unknown exception" << std::endl;
            }
        })
        .on_error([url_copy](std::string body, std::string error, unsigned status) {
            BOOST_LOG_TRIVIAL(error) << "Helio request_support_material failed - URL: " << url_copy << ", Status: " << status << ", Error: " << error << ", Body: " << body;
            std::cerr << "[HELIO ERROR] request_support_material failed - URL: " << url_copy << ", Status: " << status << ", Error: " << error << ", Body: " << body << std::endl;
        })
        .perform();
}
...
std::string HelioQuery::get_helio_api_url()
{
    ...
    // Fallback defaults (helps fresh installs where helio_api_* keys are unset).
...
HelioQuery::UploadFileResult HelioQuery::upload_file_to_presigned_url(const std::string file_path_string, const std::string upload_url)
{
    ...
    BOOST_LOG_TRIVIAL(warning) << "[HELIO DEBUG] upload_file_to_presigned_url - file: " << file_path_string 
                                << ", upload_url length: " << upload_url.length();
    std::cerr << "[HELIO DEBUG] upload_file_to_presigned_url - file: " << file_path_string 
              << ", upload_url length: " << upload_url.length() << std::endl;

    Http http = Http::put(upload_url);
    http.header("Content-Type", "application/octet-stream")
        .header("HelioAdditive-Client-Name", SLIC3R_APP_NAME)
        .header("HelioAdditive-Client-Version", GUI::VersionInfo::convert_full_version(SLIC3R_VERSION))
        .timeout_connect(60)      // 60 seconds to establish connection
        .timeout_max(600);        // 10 minutes total timeout for large file uploads
    ...
    if (!fs::exists(file_path)) {
        res.success = false;
        res.error = (boost::format("File does not exist: %1%") % file_path_string).str();
        BOOST_LOG_TRIVIAL(error) << "[HELIO ERROR] " << res.error;
        std::cerr << "[HELIO ERROR] " << res.error << std::endl;
        return res;
    }
    ...
    try {
        ...
        BOOST_LOG_TRIVIAL(warning) << "[HELIO DEBUG] File size: " << file_size << " bytes";
        std::cerr << "[HELIO DEBUG] File size: " << file_size << " bytes" << std::endl;
    } catch (...) {
        BOOST_LOG_TRIVIAL(warning) << "[HELIO DEBUG] Could not get file size";
        std::cerr << "[HELIO DEBUG] Could not get file size" << std::endl;
    }
    ...
    http.set_put_body(file_path)
        .on_progress([&res](Http::Progress progress, bool& cancel) {
            // Progress callback to keep connection alive and log upload progress
            if (progress.ultotal > 0) {
                float percent = static_cast<float>(progress.ulnow) / progress.ultotal * 100.0f;
                static float last_logged_percent = -1.0f;
                // Log every 10% to avoid spam
                if (percent - last_logged_percent >= 10.0f || percent >= 100.0f) {
                    BOOST_LOG_TRIVIAL(warning) << "[HELIO DEBUG] Upload progress: " << percent << "% (" 
                                               << progress.ulnow << "/" << progress.ultotal << " bytes)";
                    std::cerr << "[HELIO DEBUG] Upload progress: " << percent << "% (" 
                              << progress.ulnow << "/" << progress.ultotal << " bytes)" << std::endl;
                    last_logged_percent = percent;
                }
            }
            cancel = false; // Don't cancel
        })
        .on_header_callback([&res](std::string header_line) {
            std::string lower_line;
            for (unsigned char c : header_line) {
        ...
        .on_complete([&res](std::string body, unsigned status) {
            BOOST_LOG_TRIVIAL(warning) << "[HELIO DEBUG] Upload on_complete - status: " << status 
                                       << ", body length: " << body.length();
            std::cerr << "[HELIO DEBUG] Upload on_complete - status: " << status 
                      << ", body length: " << body.length() << std::endl;
            if (status == 200)
                res.success = true;
            else {
                res.success = false;
                res.error   = (boost::format("status: %1%, body: %2%") % status % body).str();
                BOOST_LOG_TRIVIAL(error) << "[HELIO ERROR] Upload failed: " << res.error;
                std::cerr << "[HELIO ERROR] Upload failed: " << res.error << std::endl;
            }
        })
        .on_error([&res](std::string body, std::string error, unsigned status) {
            res.success = false;
            res.error   = (boost::format("status: %1%, error: %2%, body: %3%") % status % error % body).str();
            BOOST_LOG_TRIVIAL(error) << "[HELIO ERROR] Upload on_error: " << res.error;
            std::cerr << "[HELIO ERROR] Upload on_error: " << res.error << std::endl;
        })
        .perform_sync();
...
void HelioBackgroundProcess::helio_threaded_process_start(std::mutex&                                slicing_mutex,
                                                          ...
                                                          std::unique_ptr<GUI::NotificationManager>& notification_manager)
{
    // Check if already canceled before setting to RUNNING
    if (!was_canceled()) {
        set_state(STATE_RUNNING);
    } else {
        BOOST_LOG_TRIVIAL(warning) << "[HELIO DEBUG] helio_threaded_process_start() - Process was canceled before starting, not setting to RUNNING";
        std::cerr << "[HELIO DEBUG] helio_threaded_process_start() - Process was canceled before starting, not setting to RUNNING" << std::endl;
        return;
    }
    ...
    if ((slicing_state == BackgroundSlicingProcess::STATE_FINISHED || slicing_state == BackgroundSlicingProcess::STATE_IDLE) &&
        ...
        // DEBUG: Log the gcode result being used for upload
        std::cerr << "[HELIO_DEBUG] helio_threaded_process_start: m_gcode_result pointer = " << (void*)m_gcode_result << std::endl;
        if (m_gcode_result) {
            std::cerr << "[HELIO_DEBUG] helio_threaded_process_start: m_gcode_result->filename = " << m_gcode_result->filename << std::endl;
            std::cerr << "[HELIO_DEBUG] helio_threaded_process_start: m_gcode_result->moves.size() = " << m_gcode_result->moves.size() << std::endl;
        }
        ...
        if (create_presigned_url_res.error.empty() && create_presigned_url_res.status == 200 && !was_canceled()) {
            ...
            // DEBUG: Log what file is being uploaded and its size
            std::cerr << "[HELIO_DEBUG] helio_threaded_process_start: UPLOADING file = " << m_gcode_result->filename << std::endl;
            
            // Get file size to verify content
            boost::filesystem::path upload_file_path(m_gcode_result->filename);
            if (boost::filesystem::exists(upload_file_path)) {
                auto file_size = boost::filesystem::file_size(upload_file_path);
                std::cerr << "[HELIO_DEBUG] helio_threaded_process_start: Upload file size = " << file_size << " bytes" << std::endl;
            } else {
                std::cerr << "[HELIO_DEBUG] helio_threaded_process_start: WARNING - Upload file does not exist!" << std::endl;
            }
            ...
            BOOST_LOG_TRIVIAL(warning) << "[HELIO DEBUG] File upload result - success: " << upload_file_res.success 
                                       << ", error: " << upload_file_res.error 
                                       << ", trace_id: " << upload_file_res.trace_id;
            std::cerr << "[HELIO DEBUG] File upload result - success: " << upload_file_res.success 
                      << ", error: " << upload_file_res.error 
                      << ", trace_id: " << upload_file_res.trace_id << std::endl;
            ...
            if (upload_file_res.success && !was_canceled()) {
                ...
                std::string error_message = _u8L("Helio: file upload failed");
...
void HelioBackgroundProcess::create_simulation_step(HelioQuery::CreateGCodeResult create_gcode_res, std::unique_ptr<GUI::NotificationManager>& notification_manager)
{
    if (create_gcode_res.success && !was_canceled()) {
        ...
        std::cerr << "[HELIO_DEBUG] create_simulation_step: gcode_id = " << gcode_id << std::endl;
        ...
        std::cerr << "[HELIO_DEBUG] create_simulation_step: NEW simulation_id = " << create_simulation_res.id << std::endl;
        std::cerr << "[HELIO_DEBUG] create_simulation_step: OLD current_simulation_result.id = " << current_simulation_result.id << std::endl;
        ...
        if (create_simulation_res.success && !was_canceled()) {
            ...
            std::cerr << "[HELIO_DEBUG] create_simulation_step: Starting progress polling with simulation_id = " << create_simulation_res.id << std::endl;
            ...
            while (!was_canceled()) {
                ...
                std::cerr << "[HELIO_DEBUG] create_simulation_step: poll response - is_finished=" << check_simulation_progress_res.is_finished 
                          << " progress=" << check_simulation_progress_res.progress 
                          << " id=" << check_simulation_progress_res.id << std::endl;
                ...
                if (check_simulation_progress_res.status == 200) {
                    ...
                    if (check_simulation_progress_res.error.empty()) {
                        ...
                        if (check_simulation_progress_res.is_finished) {
                            ...
                            std::cerr << "[HELIO_DEBUG] create_simulation_step: Simulation finished!" << std::endl;
                            std::cerr << "[HELIO_DEBUG] create_simulation_step: m_gcode_result->filename = " << m_gcode_result->filename << std::endl;
                            std::cerr << "[HELIO_DEBUG] create_simulation_step: check_simulation_progress_res.url = " << check_simulation_progress_res.url << std::endl;
                            
                            std::string simulated_gcode_path = HelioBackgroundProcess::create_path_for_simulated_gcode(
                                m_gcode_result->filename);
                            std::cerr << "[HELIO_DEBUG] create_simulation_step: simulated_gcode_path = " << simulated_gcode_path << std::endl;
                ...
                times_queried++;
                // Make sleep interruptible by checking cancellation every 0.5 seconds
                for (int i = 0; i < 6 && !was_canceled(); ++i) {
...
void HelioBackgroundProcess::save_downloaded_gcode_and_load_preview(std::string                                file_download_url,
                                                                    ...
                                                                    HelioQuery::RatingData                     rating_data)
{
    // DEBUG: Log the download parameters
    std::cerr << "[HELIO_DEBUG] save_downloaded_gcode_and_load_preview: file_download_url = " << file_download_url << std::endl;
    std::cerr << "[HELIO_DEBUG] save_downloaded_gcode_and_load_preview: helio_gcode_path (where to save) = " << helio_gcode_path << std::endl;
    std::cerr << "[HELIO_DEBUG] save_downloaded_gcode_and_load_preview: tmp_path (original gcode path) = " << tmp_path << std::endl;
    ...
    while (response_status != 200 && !was_canceled()) {
        if (number_of_seconds_till_next_attempt <= 0) {
            ...
            // Make sleep interruptible by checking cancellation every 0.1 seconds
            for (int i = 0; i < 10 && !was_canceled(); ++i) {
    ...
    if (response_error.empty() && !was_canceled()) {
        std::cerr << "[HELIO_DEBUG] save_downloaded_gcode_and_load_preview: Downloaded gcode size = " << downloaded_gcode.size() << " bytes" << std::endl;
        std::cerr << "[HELIO_DEBUG] save_downloaded_gcode_and_load_preview: Saving to helio_gcode_path = " << helio_gcode_path << std::endl;
...
{
    // DEBUG: Log what file is being processed
    std::cerr << "[HELIO_DEBUG] load_helio_file_to_viwer: file_path (downloaded helio result) = " << file_path << std::endl;
    std::cerr << "[HELIO_DEBUG] load_helio_file_to_viwer: tmp_path (original sliced gcode) = " << tmp_path << std::endl;
    std::cerr << "[HELIO_DEBUG] load_helio_file_to_viwer: m_gcode_result BEFORE processing pointer = " << (void*)m_gcode_result << std::endl;
    if (m_gcode_result) {
        std::cerr << "[HELIO_DEBUG] load_helio_file_to_viwer: m_gcode_result BEFORE filename = " << m_gcode_result->filename << std::endl;
        std::cerr << "[HELIO_DEBUG] load_helio_file_to_viwer: m_gcode_result BEFORE moves.size() = " << m_gcode_result->moves.size() << std::endl;
    }
    ...
    std::cerr << "[HELIO_DEBUG] load_helio_file_to_viwer: m_gcode_result AFTER processing pointer = " << (void*)m_gcode_result << std::endl;
    std::cerr << "[HELIO_DEBUG] load_helio_file_to_viwer: m_gcode_result AFTER filename = " << m_gcode_result->filename << std::endl;
    std::cerr << "[HELIO_DEBUG] load_helio_file_to_viwer: m_gcode_result AFTER moves.size() = " << m_gcode_result->moves.size() << std::endl;

    set_state(STATE_FINISHED);

    std::cerr << "[HELIO_DEBUG] load_helio_file_to_viwer: Posting HelioCompletionEvent with file_path=" << file_path << " tmp_path=" << tmp_path << std::endl;
