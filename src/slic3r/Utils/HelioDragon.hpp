...
public:
    ...
    static std::string generateTimestampedString()
    {
        ...
        return "CrealityPrint " + iso_datetime;
    }
...
public:
    ...
    void stop()
    {
        ...
        m_mutex.unlock();
        BOOST_LOG_TRIVIAL(warning) << "[HELIO DEBUG] HelioBackgroundProcess::stop() called - state changed from " << old_state << " to " << STATE_CANCELED;
        std::cerr << "[HELIO DEBUG] HelioBackgroundProcess::stop() called - state changed from " << old_state << " to " << STATE_CANCELED << std::endl;
    }
    ...
    bool is_running()
    {
        ...
        int current_state = m_state;
        m_mutex.unlock();
        BOOST_LOG_TRIVIAL(warning) << "[HELIO DEBUG] HelioBackgroundProcess::is_running() - state: " << current_state << ", returning: " << running_state;
        std::cerr << "[HELIO DEBUG] HelioBackgroundProcess::is_running() - state: " << current_state << ", returning: " << running_state << std::endl;
    ...
    bool was_canceled()
    {
        ...
        m_mutex.unlock();
        // Removed verbose debug logging - was called too frequently during polling
    ...
    State get_state()
    {
        ...
        auto state = m_state;
        m_mutex.unlock();
        // Note: get_state() is called frequently, so we don't log every call
        return state;
    }
    ...
    void reset()
    {
        ...
        // FIX: Also reset the simulation/optimization result IDs to prevent stale server responses
        std::cerr << "[HELIO DEBUG] HelioBackgroundProcess::reset() - clearing current_simulation_result.id = " << current_simulation_result.id << std::endl;
        std::cerr << "[HELIO DEBUG] HelioBackgroundProcess::reset() - clearing current_optimization_result.id = " << current_optimization_result.id << std::endl;
        current_simulation_result.reset();
        current_optimization_result.reset();
        
        BOOST_LOG_TRIVIAL(warning) << "[HELIO DEBUG] HelioBackgroundProcess::reset() called - state changed from " << old_state << " to " << STATE_INITIAL;
        std::cerr << "[HELIO DEBUG] HelioBackgroundProcess::reset() called - state changed from " << old_state << " to " << STATE_INITIAL << std::endl;
    }
