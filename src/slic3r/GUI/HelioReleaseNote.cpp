    ...
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, _L("Third-Party Extension"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
     ...
     std::string icon_path = (boost::format("%1%/images/CrealityPrint.ico") % resources_dir()).str();
...
void HelioStatementDialog::on_confirm(wxMouseEvent& e)
 {
    ...
    if (page1_agree && page2_agree) {
        ...
        // Note: expand_program_holder and expand_helio_id don't exist in CrealityPrint's MainFrame
        // This functionality may need to be implemented differently or removed
...
void HelioStatementDialog::report_consent_install()
{
    ...
    // Note: report_consent doesn't exist in CrealityPrint's NetworkAgent or GUI_App
    // This functionality may need to be implemented differently or removed
    ...
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, wxString("Helio Additive"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    // Note: DevPrinterConfigUtil doesn't exist in CrealityPrint
    // Defaulting to no chamber support - this may need to be configured differently
    is_no_chamber = true; // Default assumption
    ...
    float min_speed = 0.0;
    float max_speed = 0.0;
    // Note: Using GCodeViewer::EViewType::Feedrate instead of gcode::EViewType::Feedrate
    ...
    // Note: Using GCodeViewer::EViewType::VolumetricRate instead of gcode::EViewType::VolumetricRate
    ...
    /*confirm*/
    wxBoxSizer* button_sizer = new wxBoxSizer(wxHORIZONTAL);
    // AMS_CONTROL_BRAND_COLOUR is wxColour(0, 174, 66) in BambuStudio
                                                ...
                                                const std::vector<std::shared_ptr<TextInputValChecker>>& checkers)
{
    ...
    TextInput *m_input_item = new TextInput(parent, wxEmptyString, wxEmptyString, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(120), -1), wxTE_PROCESS_ENTER);
    // Note: unit parameter not supported in CrealityPrint's TextInput - may need to add label or handle differently
