    ...
    : wxWindow(parent, id, pos, size)
{
    ...
    Bind(wxEVT_LEFT_DOWN, [this](auto& e) {
        // Only post event if button is enabled
        bool enabled = IsEnabled();
        BOOST_LOG_TRIVIAL(warning) << "[HELIO DEBUG] ExpandButton::OnLeftDown - id: " << GetId() << ", enabled: " << enabled;
        std::cerr << "[HELIO DEBUG] ExpandButton::OnLeftDown - id: " << GetId() << ", enabled: " << enabled << std::endl;
        if (!enabled) {
            std::cerr << "[HELIO DEBUG] ExpandButton click blocked - button disabled" << std::endl;
...
{
    m_bmp_str = bmp;  // Update the stored bitmap name for rescaling
...
bool ExpandButtonHolder::IsExpandButtonEnabled(wxWindowID id)
{
    wxWindowList& children = this->GetChildren();
    for (wxWindowList::iterator it = children.begin(); it != children.end(); ++it)
    {
